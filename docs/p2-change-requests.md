...
**`/cdd-new` 一句話描述：**
> 新增 parsers/layout_detector.py，以本地 Docling heron-101（ONNX，docling-project/docling-layout-heron-onnx）將 PDF 頁面切成 typed regions（text/title/table/figure/formula/header/footer/list）並寫入 IR 的 reading_order，取代脆弱的 round(y0,10pt) 啟發式；模型一律本地推論、不上傳頁面影像。
...
> **模型選型說明（2026/06 調查）**：原計畫使用 DocLayout-YOLO（0.91 mAP 係在自有 DocStructBench 上）。在同一 DocLayNet benchmark 上，DocLayout-YOLO 79.7% vs heron-101 78.0%，性能相當。heron-101 改採原因：官方直接提供 ONNX 權重（無需手動匯出）、僅需 `onnxruntime` 無需 ultralytics、IBM Docling 持續維護、DocLayout-YOLO 自 2024Q4 後無主動演進。
...
- 改善計畫 §2.3、§6.2、§6.3、§5.1 風險表（原文提及 DocLayout-YOLO 0.91 mAP；已換模型，見上方選型說明）
