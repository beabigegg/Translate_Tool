...
- `mock.patch` must target the name as bound in the **consumer** module (e.g. `services.translation_service.translate_blocks_batch`), not the definition path — Python binds names at import time; patching the source path silently misses calls through the already-imported reference — see `tests/test_translation_strategy.py` Doc2Doc mock pattern.
