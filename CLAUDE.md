...
- Tautological tests come in two forms: (1) **call-wiring** — `mock.patch` each backend separately; calling the component against itself always passes even when backends are unwired; (2) **selection** — assert WHICH element was chosen (e.g. `element_id` of the boundary element), not just HOW MANY — count-only assertions pass even when selection logic is broken — see `tests/test_renderer_convergence.py::TestLayoutEquivalence` (wiring) and `tests/test_doc_chunker.py::TestBoundaryPriority` (selection).
