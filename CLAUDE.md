...
- Tautological tests come in three forms: (1) **call-wiring** — `mock.patch` each backend separately; calling the component against itself always passes even when backends are unwired; (1b) **wrong entry point** — calling a higher-level wrapper that doesn't reach the target hook/seam (e.g. testing a processor hook via `translate_document()` which is unwired) trivially passes; (2) **selection** — assert WHICH element was chosen, not just HOW MANY — see `tests/test_renderer_convergence.py::TestLayoutEquivalence` (wiring), `tests/test_doc_chunker.py::TestBoundaryPriority` (selection), `tests/test_translation_strategy.py::test_qe_hook_called_after_translation` (wrong entry point).
...
- Bug-fix-lane gate needs `agent-log/bug-fix-engineer.yml` with a `bug-fix:` block whose `test-reproduced` reproduction points at a genuinely FAILED pre-fix `cdd-kit test run`, and whose reproduction/regression `command` equals the referenced run's recorded command minus runner-added flags — recipe: temporarily restore the pre-fix file (`git show main:<file> > <file>`), run ONLY the repro test via `cdd-kit test run --phase <p>`, then restore the fix and re-run that phase green (the failed run-dir persists for the reference); see cdd-kit README bug-fix evidence rules (ADR 0006 §6/§7).
...
- Adding an additive optional kwarg/callback to the judge/critique seams (`run_judge_loop` cancel_event/snapshot_cb, `_batched_critique_adopt` on_scored, `translation_service.status_callback`) predictably breaks test doubles that reproduce the signature — fake `run_loop`/`side_effect` closures in `test_orchestrator_judge.py`, adopt lambdas in `test_fewshot_glossary.py` — and adding `JobStatus`/`JobRecord` fields breaks `_make_job()`-style MagicMock helpers (pydantic validation). Grep for these fakes and update them IN THE SAME change (recurred in qa-judge-hang-recovery, batch-critique-qe-scoring, translation-progress-detail-ui).
- No-shell planning agents (spec-architect, test-strategist, implementation-planner) can assert a plausible but nonexistent seam/symbol even when `.cdd/code-map.yml` is accurate (pattern-matched from contract/change-request prose, not read from source) — the first shell-capable agent (implementation-planner/backend-engineer) MUST verify every named seam/symbol against live source before wiring and correct the contract/design when it's wrong — see `contracts/business/business-rules.md` BR-106 (seam-name correction) and the `layout-qa-safety-net` implementation-planner log.
