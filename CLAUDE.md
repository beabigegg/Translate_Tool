...
- `cdd-kit gate` tier-floor false-positives: common triggers include `"api key"`, `"authentication"`, `"cache"`, `"endpoint"`, `"integration"`, `"alter table"` (even phrased as "No ALTER TABLE needed"), `"breaking change"`, `"session"` — any auth-vocab, cache-vocab, migration-vocab, or routine feature-add term can force Tier 0/2 even when no actual migration is involved; always use `tier-floor-override` with written rationale; see `contracts/env/env-contract.md` Secret Policy.
