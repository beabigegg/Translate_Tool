...
- At `/cdd-close`: remove the archived change's `cdd-kit gate <id>` line AND any per-change targeted-test steps from `.github/workflows/contract-driven-gates.yml` — archived dirs no longer exist under `specs/changes/` and CI fails with "change not found".
