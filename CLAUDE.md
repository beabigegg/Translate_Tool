...
- `mock.patch` must target the name as it is bound **at the moment of the call**: for module-level imports patch the consumer module; for lazy (inside-function) imports patch the definition module, because no consumer-module binding exists at patch time — see `tests/test_translation_strategy.py` (module-level) and `tests/test_orchestrator_phase0.py` (lazy-import) for both patterns.
