...
- `cdd-kit gate` tier-floor false-positives: common triggers include `"api key"`, `"authentication"`, `"cache"`, `"endpoint"`, `"integration"` — any auth-vocab, cache-vocab, or routine feature-add term can force Tier 0/2 even when no auth system, shared cache, or migration is involved; always use `tier-floor-override` with written rationale; see `contracts/env/env-contract.md` Secret Policy.
