...
The system SHALL default to Ollama backend with TranslateGemma:12b model when available.

#### Scenario: Application startup with Ollama available
- **GIVEN** Ollama service is running
- **WHEN** the application starts
- **THEN** the backend dropdown SHALL default to "Ollama"
- **AND** the model dropdown SHALL default to "translategemma:12b" if available

#### Scenario: Application startup without Ollama
- **GIVEN** Ollama service is not running
- **WHEN** the application starts
- **THEN** the backend dropdown SHALL still default to "Ollama"
- **AND** display a warning that Ollama is not available
- **AND** allow user to switch to Dify backend
