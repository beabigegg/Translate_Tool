...
| BR-38 | no-silent-truncation | application-team | Every truncation of a `TranslatableElement` during rendering MUST set `render_truncated = True` on that IR element — including cascade step (e) truncation AND the render-time overflow guard dropping lines. Silent truncation (text clipped without marking) is forbidden. On the PDF-to-PDF path, marked truncation MUST be surfaced post-render as a job warning (`render_truncation_warning`) via `warnings_callback`. `render_truncated` is consumed by the QA safety net and human-review tooling. | tests/test_text_region_renderer.py, tests/test_layout_confirmation_warnings.py |
...
| BR-92 | qe-rescore-threshold | application-team | Segments whose post-job CometKiwi score falls below QE_RESCORE_THRESHOLD (default 0.5; type float in [0.0, 1.0]) are flagged for re-translation by the post-translate hook (AC-2). Has no effect when QE_ENABLED=false. The threshold is model-scale dependent and MUST be tunable via env var; the default 0.5 is a conservative placeholder that MUST be calibrated for the deployment model. | tests/test_quality_evaluation.py |
