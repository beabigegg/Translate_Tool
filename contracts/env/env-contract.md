...
| QE_ENABLED | backend | all | no | no | true | true | application-team | boolean (true/false or 1/0) | yes | When false (or 0), QE scoring step is skipped entirely; GET /jobs/{id}/quality returns status: "disabled". Default changed to true (quality-metrics-gating AC-3) so the critique adoption gate (AC-7) and post-job rescore threshold (AC-2) are active out of the box. Falls back to length-ratio heuristic when COMET library is absent (AC-8). Set to false to opt out. See BR-57, BR-91. |
| QE_RESCORE_THRESHOLD | backend | all | no | no | 0.5 | 0.5 | application-team | float in [0.0, 1.0] | no | Segments whose post-job CometKiwi score falls below this value are flagged for re-translation in the post-translate hook (AC-2). Has no effect when QE_ENABLED=false. Model-scale dependent — tune for your deployment. Default 0.5 is a conservative placeholder. See BR-92. |
